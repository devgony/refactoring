...
- 중복 부분이 같은 class 내 subclass 로 존재한다면 Pull Up Method
...
- 추출된 함수에 너무 많은 인자를 전달하면 가독성이 더 떨어진다: `Introduce Parameter Object` and `Preserve Whole Object`
- 조건문: `Decompose Conditional`
- 같은 조건에 대한 스위치문이 한 개 이상 있다면: `Replace Conditional with Polymorphism`
- 반복문 역시 함수로 추출, 추출이 어려운 경우는 두 가지 이상의 일을 하고있기 때문: `Split Loop`

# Long Parameter List

> 과거에는 global data 를 쓰는것보다는 인자로 다 넘기는게 좋다고 했다. 하지만 인자가 너무 많아지면 그것 또한 문제가 된다.

- `Replace Parameter with Query`: 종속성이 있는 다른 인자를 제거하고 함수 내에서 계산
- `Preserve Whole Object`: 객체 전체를 전달하고 필요한 값만 추출
- `Introduce Parameter Object`: 항상 같이 쓰이는 인자들을 객체로 묶어 전달
- `Remove Flag Argument`: boolean 인자를 제거하고 서로다른 행동을 두 함수로 분리
- `Combine Functions into Class`: 여러 함수들이 같은 인자를 공유한다면 클래스로 묶는다
  - 함수형의 경우 `Partially applied function` 을 활용할 수 있다

# Global Data

- 전역 변수는 어느 곳에서나 변경될 수 있어 버그가 어디서 발생했는지 찾기 어렵다
- 전역 변수 뿐만아니라 클래스 변수, 싱글톤 역시 문제가 될 수 있다
- `Encapsulate Variable`: 변수를 감싸고, 변수를 변경하는 함수를 만들어 사용
...
- `Encapsulate Variable`: 모든 변경이 추적 가능한 근접 함수에 의해서만 이루어지도록 감싼다
...
- API 에서는 `Separate Query from Modifier` 를 통해 side effect 를 발생시키는 코드를 최대한 호출하지 않도록 한다
- 최대한 빨리 `Remove Setting Method` 를 활용하여 불필요한 setter 를 제거하면 변수의 scope 를 줄일 수 있다
- 모든 곳에서 사용할 수 있는 mutable data 는 불필요하다. `Replace Derived Variable with Query` 로 제거 하자

- 변수의 scope 이 넓지 않으면 mutable data 자체가 큰 문제가 되지는 않는다. scope 이 증가하면서 위험성도 함께 증가
- `Combine Functions into Class`, `Combine Functions into Transform`: update 에 필요한 범위 제한
- `Change Reference to Value`: 변수가 내부 구조를 가진 data 를 포함하는 경우, 일부 수정보다 전체 구조를 교체하는게 낫다
...
- 두 가지 측면이 순서를 형성하는 경우 명확한 데이터 구조로 `Split Phase`
...
- 여러 함수들이 유사한 데이터들에 대해 수행되고 있으면 `Combine Functions into Class`
- 특정 데이터 구조를 변화시키거나 enrich 하는 함수들이면 `Combine Functions into Transform`
...
- primitive type 이 조건을 제어한다면 `Replace Type Code With Subclasses` + `Replace Conditional with Polymorphism`
- Data와 함께라면 `Extract Class`, `Introduce Parameter Object`

# Repeated Switches

- switch 문이 두 번 이상 반복된다면 clause 를 추가 할 때마다 모든 switch 문을 업데이트 해주어야 함: `Replace Conditional with Polymorphism`

# Loops

- `Replace Loop with Pipeline`: 과거에는 반복문을 처리할 방법이 없었지만 현대에는 first-class function 들이 도입되면서 반복문을 함수로 추출할 수 있게 되었다

# Lazy Element

- 별도의 구조가 불필요한 경우도 있다
- 본질적으로 단순한 함수/클래스 이거나, 로직이 복잡해질 것으로 예상했으나 실제로 그렇지 않은경우, 한 때는 제 역할을 했지만 리팩토링으로 규모가 축소된 경우에는 과감하게 없앨수도 있어야한다.
- `Inline Function`, `Inline Class`, `Callapse Hierarchy`

# Speculative Generality

- 미래에 사용할 것이라고 추측하는 코드는 불필요하다
- abstract classs 가 하는 일이 없다면 `Collapse Hierarchy`
...
- `Introduce Sepacial Case` 로 대체 클래스를 만들어 조건부 코드 제거 가능
...
- 너무 많은 위임이 발생하는 경우 추적이 어려우므로: `Remove Middle Man`
- 일부 메서드만 위임하는 경우 `Inline Function` 으로 중간 객체를 제거한다
- 추가 로직이 있는 경우 `Replace Superclass with Delegate` 혹은 `Replace Subclass with Delegate` 를 통해 middle man 을 실제 Object 로 변경한다

# Insider Trading

- 모듈 사이의 데이터 교환은 최소화 해야 한다
- 데이터 교환이 많은 모듈은 `Move Function`, `Move Field` 를 통해 분리해서 데이터 교환을 최소화 한다
- 모듈이 공통의 관심사를 가지고 있다면 세 번째 모듈을 만들어서 공통 관심사를 분리하거나, `Hide Delegate` 를 통해 다른 모듈이 중개자 역할을 하도록 한다
- 상속은 담합으로 이어질 수 있으므로 `Replace Subclass with Delegate` 혹은 `Replace Superclass with Delegate` 를 통해 상속을 제거한다
...
- 코드 가 긴 경우에는 클라이언트가 클래스의 하위 집합을 사용하는지 확인.
  - `Extract Class`, `Extract Superclass`, or `Replace Type Code with Subclasses` 사용하여 분리
...
- public field를 가지고 있다면 즉시 `Encapsulate Field` 를 통해 getter, setter 를 만들어 사용
  - immutable 한 경우 setter 를 제거하고, 생성자에서 값을 설정하도록 한다
- getter, setter 를 통해 데이터 클래스를 참조하는 부분을 찾고 해당 로직을 데이터 클래스 안으로 `Move Function`
- 옮길 수 없는 경우 `Extract Function` 으로 옮길 수 있는 함수로 추출
- 단, 별개의 함수 호출에서 결과 레코드로 사용되는 경우는 예외이다.
...
- subclass 가 행동을 재활용 하고있지만 superclass 의 interface 를 벗어나려하면 `Replace Subclass with Delegate` or `Replace Superclass with Delegate` 를 로 계층 구조를 완전히 바꾼다
...
- 특정 규칙에 대한 주석을 달고 싶으면 `Introduce Assertion`
